...
export default function NewProductPage() {
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      tags: form.tags,
      images: form.image_url ? [{ color: form.color || 'Default', url: form.image_url, is_default: true }] : [],
    }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#e0e0e0', fontFamily: 'JetBrains Mono, monospace' }}>
      ...
      <div style={{ borderBottom: '1px solid #1e1e1e', padding: '20px 32px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <span style={{ fontSize: '14px', fontWeight: '600' }}>New Product</span>
      </div>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 32px' }}>
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <div style={{ background: '#2e1a1a', border: '1px solid #dd6974', color: '#dd6974', borderRadius: '8px', padding: '12px 16px', fontSize: '13px', marginBottom: '24px' }}>
              ⚠ {error}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            <div>
              ...
              <input style={inputStyle} value={form.name} onChange={e => handleChange('name', e.target.value)} placeholder="e.g. Classic Tee" required />
            </div>
            <div>
              <label style={labelStyle}>Slug *</label>
              <input style={inputStyle} value={form.slug} onChange={e => handleChange('slug', e.target.value)} placeholder="classic-tee" required />
              <div style={{ fontSize: '11px', color: '#555', marginTop: '4px' }}>/product/{form.slug || 'your-slug'}</div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <textarea style={{ ...inputStyle, minHeight: '100px', resize: 'vertical', lineHeight: '1.6' }} value={form.description} onChange={e => handleChange('description', e.target.value)} placeholder="Describe the product..." />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            <div>
              ...
              <input style={inputStyle} type="number" step="0.01" min="0" value={form.price} onChange={e => handleChange('price', e.target.value)} placeholder="45.00" required />
            </div>
            <div>
              <label style={labelStyle}>Compare Price (original)</label>
              <input style={inputStyle} type="number" step="0.01" min="0" value={form.compare_price} onChange={e => handleChange('compare_price', e.target.value)} placeholder="60.00" />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', paddingTop: '20px' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px' }}>
                <input type="checkbox" checked={form.is_featured} onChange={e => handleChange('is_featured', e.target.checked)} style={{ accentColor: '#01696f', width: '16px', height: '16px' }} />
                Featured Product
              </label>
            </div>
          </div>
              ...
              {ALL_SIZES.map(size => (
                <button
                  ...
                  style={{
                    ...
                    background: form.sizes.includes(size) ? '#01696f' : '#1e1e1e',
                    color: form.sizes.includes(size) ? '#fff' : '#666',
                    border: `1px solid ${form.sizes.includes(size) ? '#01696f' : '#2e2e2e'}`,
                  }}
                >
                  {size}
                </button>
              ...
              {['new', 'sale', 'bestseller', 'limited', 'coming-soon'].map(tag => (
                <button
                  ...
                  style={{
                    ...
                    background: form.tags.includes(tag) ? '#1a2433' : '#1e1e1e',
                    color: form.tags.includes(tag) ? '#5591c7' : '#555',
                    border: `1px solid ${form.tags.includes(tag) ? '#5591c7' : '#2e2e2e'}`,
                  }}
                >
                  {tag}
                </button>
